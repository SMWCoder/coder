terraform {
  required_providers {
    coder = {
      source  = "coder/coder"
      version = "~> 2.0"  # Compatible with Coder v2.20.3
    }
    docker = {
...
locals {
  ...
  // NOTE: keep these up to date with those in ../dogfood-envbuilder/main.tf!
  docker_host = {
  ...
  repo_dir = "/workspaces/redhat-env"
}
...
data "coder_parameter" "devcontainer_repo" {
  ...
  description = "Repo containing the Red Hat devcontainer.json. This is only cloned once."
  mutable     = false
}
...
data "coder_parameter" "devcontainer_dir" {
  ...
  default     = "dogfood/redhat-envbuilder/"
  description = "Directory containing the Red Hat devcontainer.json relative to the repository root"
  mutable     = true
}
...
resource "coder_agent" "dev" {
  ...
  startup_script = <<-EOT
    ...
    # Install additional Red Hat specific dependencies
    sudo dnf install -y epel-release
    sudo dnf install -y crypto-policies-scripts fipscheck openssl-fips
    sudo dnf update -y

    # Verify and enforce FIPS compliance
    # Check current FIPS status
    echo "Current FIPS policy: $(sudo update-crypto-policies --show)"
    
    # Ensure FIPS mode is active
    sudo update-crypto-policies --set FIPS
    echo "Updated FIPS policy: $(sudo update-crypto-policies --show)"
    
    # Verify OpenSSL is running in FIPS mode
    openssl md5 /etc/hosts 2>&1 | grep -q "disabled for FIPS" && echo "OpenSSL FIPS mode is active" || echo "WARNING: OpenSSL FIPS mode is NOT active"
    
    # Configure Node.js for FIPS compliance
    echo 'export NODE_OPTIONS="$NODE_OPTIONS --openssl-config=/etc/crypto-policies/back-ends/openssl.config"' >> ~/.bashrc
    
    # Configure SELinux in enforcing mode for FIPS compliance
    if command -v setenforce &>/dev/null; then
      sudo setenforce 1 || echo "Unable to set SELinux to enforcing mode"
      echo "SELinux status: $(getenforce)"
    fi

    # Create symlinks for FIPS-validated crypto libraries if they don't exist
    if [ ! -f /lib64/libcrypto.so.10.FIPS ]; then
      sudo ln -sf /lib64/libcrypto.so.10 /lib64/libcrypto.so.10.FIPS || true
    fi
  EOT
}
...
locals {
  cache_repo = "us-central1-docker.pkg.dev/coder-dogfood-v2/envbuilder-cache/redhat-envbuilder"
  envbuilder_env = {
    ...
    "ENVBUILDER_FALLBACK_IMAGE" : "registry.access.redhat.com/ubi9/ubi:latest", # This image runs if builds fail
